...
export default function Pricing() {
    ...
    const { hasUsedSampler, subscription, isPremium } = useSubscription();
    ...
    const allPlans = [
        ...
        },
        {
            ...
            period: '/ mo',
            cta: 'Try It Out',
            features: ['1 Account | 10 Autos', '10,000 DMs / mo', 'Growth Tool (Ask to follow)', 'Carousel & Post Auto', 'Lead Manager (CRM)', 'Follow-ups & Appointments'],
        ...
        },
        {
            ...
            period: '/ mo',
            cta: 'Get Premium',
            features: ['1 Instagram Account', 'Unlimited Automations', 'Unlimited DMs', 'Growth Tool (Ask to follow)', 'Live & Story automation', 'Analytics | Priority Support'],
        ...
        },
        {
            ...
            period: '/ mo',
            cta: 'Go Pro',
            features: ['Up to 2 Accounts', 'Unlimited Scale', 'All Premium Features', 'Lead Manager (Full CRM)', 'Carousel & Follow-ups', 'Appointment System'],
            highlighted: true,
        ...
        },
        {
            ...
            period: '',
            cta: 'Contact Sales',
            features: ['Multiple Accounts', 'Dedicated Manager', 'White-label options', 'SLA & Priority Support', 'Custom Integrations'],
    ...
    const paidPlans = allPlans.filter(p => {
        ...
        return p.type === 'paid' && !isSamplerAndUsed;
    });

    const isActivePlan = (planId: string) => planId === activePlanId;
        ...
        <div className="min-h-screen w-full bg-white text-gray-900 font-sans flex flex-col md:flex-row overflow-y-auto md:overflow-hidden">
            ...
            <div className="w-full md:w-[32%] md:min-w-[340px] border-b md:border-b-0 md:border-r border-gray-100 p-6 md:p-8 flex flex-col justify-between relative bg-slate-50/50">
                <div className="space-y-6 md:space-y-8">
                    ...
                    <div>
                        ...
                        {isPremium ? (
                            <div className="flex items-center gap-2 mt-2">
                                ...
                                <p className="text-[11px] text-amber-600 font-black uppercase tracking-widest">Active Subscription</p>
                            </div>
                        ) : (
                    ...
                    {/* Free Plan Card */}
                    <div className={`bg-white rounded-3xl p-5 md:p-6 border transition-all relative overflow-hidden group ${
                        isActivePlan('basic')
                            ? 'border-blue-400 shadow-md shadow-blue-100 ring-1 ring-blue-400/20'
                            : 'border-gray-100 shadow-sm hover:border-blue-200'
                    }`}>
                        {isActivePlan('basic') && (
                            <div className="absolute top-3 right-3">
                                <span className="bg-blue-600 text-[8px] text-white font-black uppercase tracking-widest px-2.5 py-1 rounded-full flex items-center gap-1">
                                    <Crown className="w-2.5 h-2.5" /> Current Plan
                                </span>
                            </div>
                        )}
                        <div className="flex justify-between items-start mb-4">
                            <div className="p-2 bg-gray-50 rounded-xl group-hover:bg-blue-50 transition-colors">{freePlan.icon}</div>
                        ...
                        {isActivePlan('basic') ? (
                            <button onClick={() => navigate('/dashboard')} className="w-full py-3 rounded-xl bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-colors">
                                Go to Dashboard
                            </button>
                        ) : (
                            <button onClick={() => navigate('/dashboard')} className="w-full py-3 rounded-xl bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest hover:bg-gray-800 transition-colors">
                ...
                <div className="space-y-4 pt-8 md:pt-0">
                    <p className="text-[10px] text-gray-400 font-bold text-center leading-relaxed">Trusted by 10,000+ creators and businesses worldwide</p>
                    <button onClick={() => navigate('/billing')} className="w-full text-center text-[10px] font-black text-blue-500 hover:text-blue-700 transition-colors uppercase tracking-widest">
                        Manage Billing →
                    </button>
                    <button onClick={() => signOut()} className="w-full text-center text-[10px] font-black text-gray-400 hover:text-red-500 transition-colors uppercase tracking-widest">
                        Log out
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col p-6 md:p-8 bg-white overflow-y-auto">
                <div className="flex flex-col items-center justify-start h-fit max-w-5xl mx-auto w-full">
                    {/* Billing Toggle */}
                    <div className="flex items-center justify-center gap-6 mb-8 md:mb-12 bg-gray-50 px-6 py-3 rounded-full border border-gray-100 shadow-sm mt-4 md:mt-0">
                        ...
                        <button onClick={() => setBillingCycle(billingCycle === 'annual' ? 'quarterly' : 'annual')} className="w-12 h-6 bg-gray-200 rounded-full p-1 relative transition-colors shadow-inner">
                    ...
                    {/* Paid Plans Grid */}
                    <div className={`grid gap-6 w-full h-fit py-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3`}>
                        {paidPlans.map((plan) => {
                            ...
                            return (
                                <div key={plan.id} className={`group relative rounded-3xl bg-white border p-6 flex flex-col transition-all duration-300 ${
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div className={`p-2 rounded-xl ${isCurrent ? 'bg-blue-50' : plan.highlighted ? 'bg-purple-50' : 'bg-gray-50'}`}>{plan.icon}</div>
                                        <div className="text-right">
                                    ...
                                    <ul className="space-y-2 mb-6 flex-1">
                                        {plan.features.slice(0, 6).map((feature, i) => (
                                            <li key={i} className="flex items-start gap-2">
                                                <Check className={`w-3 h-3 mt-0.5 shrink-0 ${isCurrent ? 'text-blue-600' : plan.highlighted ? 'text-purple-600' : 'text-blue-500'}`} />
                                    ...
                                    {isCurrent ? (
                                        <button onClick={() => navigate('/billing')} className="w-full py-3 rounded-xl text-[9px] font-black transition-all uppercase tracking-widest bg-blue-600 text-white hover:bg-blue-700">
                                            Manage Plan →
                                        </button>
                                    ) : (
                                        <button
