                  ...
                  {loadingMedia ? (
                    ...
                    <div className="grid grid-cols-2 auto-rows-max gap-2 max-h-[310px] overflow-y-auto pr-1">
                      {posts.map(post => {
                        ...
                        return (
                          <div key={post.id} onClick={() => toggleMediaSelection(post.id)} className={`relative w-full pb-[100%] cursor-pointer rounded-xl overflow-hidden border-2 transition-all ${isSelected ? 'border-purple-600' : 'border-transparent hover:border-purple-200'}`}>
