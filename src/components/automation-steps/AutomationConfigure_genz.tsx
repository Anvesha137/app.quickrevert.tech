                  ...
                  {loadingMedia ? (
                    ...
                    <div className="grid grid-cols-2 gap-2 max-h-[310px] overflow-y-auto pr-1">
                      {posts.map(post => {
                        ...
                        return (
                          <div key={post.id} onClick={() => toggleMediaSelection(post.id)} className={`relative aspect-square w-full cursor-pointer rounded-xl overflow-hidden border-2 transition-all ${isSelected ? 'border-purple-600' : 'border-transparent hover:border-purple-200'}`}>
                            {post.media_type === 'VIDEO' ? <video src={post.media_url} poster={post.thumbnail_url} autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover" /> : <img src={post.media_url} alt="" className="absolute inset-0 w-full h-full object-cover" />}
