...
const DEFAULT_TEASER_MESSAGE = "Hey! Glad youΓÇÖre here... Tap below and IΓÇÖll send you a message shortly ≡ƒæÇ";
const DEFAULT_NOT_FOLLOWING_MESSAGE = "Oops! Looks like you haven't followed me yet ≡ƒæÇ...";
const DEFAULT_TEASER_BTN_TEXT = "Send Access";
const DEFAULT_VERIFY_BTN_TEXT = "I've Followed! Γ£à";
const OLD_DEFAULT_TITLE = 'Hey! Thanks for your comment so much. Here is the link you asked for...';
const NEW_DEFAULT_TITLE = 'Hey! Thanks so much for your comment ≡ƒÆî EverythingΓÇÖs been sent your way Γ£¿';
...
export default function ActionConfig({ triggerType, actions, onActionsChange, onSave, saving, readOnly }: ActionConfigProps) {
  ...
  const handleConnectGoogle = async () => {
    ...
    const callbackUrl = `${window.location.origin}/dashboard/automations`;
    const authUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/auth-google-init?userId=${user.id}&redirect=${encodeURIComponent(callbackUrl)}`;
  ...
  const toggleReply = () => {
    ...
    if (hasReply) {
      ...
      onActionsChange([...actions, {
        ...
        replyTemplates: [
          'Ayyy check your DMs ≡ƒæÇΓ£¿',
          'Just dropped you a message ≡ƒÆî≡ƒöÑ',
          'Doneee, sent you the details ≡ƒ½╢≡ƒô⌐',
          'You got a lil surprise in your inbox ≡ƒÿî≡ƒÆ½'
        ],
        actionButtons: []
  ...
  const isDmValid = dmAction 
    ...
    : true;
  const isSheetValid = !!leadAction?.spreadsheetUrl && leadAction.spreadsheetUrl.includes('docs.google.com/spreadsheets');
  const isLeadValid = leadAction?.enabled ? (isSheetValid && isGoogleConnected === true) : true;
  const canSave = actions.length > 0 && isReplyValid && isDmValid && isLeadValid;
      ...
      <div>
        <div className="flex items-start gap-3 md:gap-4 mb-3 md:mb-4">
          ...
          <div className="pt-0.5 md:pt-1">
            ...
            <p className={`text-xs md:text-sm font-medium leading-relaxed mt-0.5 ${darkMode ? 'text-white/40' : 'text-gray-400'}`}>Turn on one or both ΓÇö they run together when the trigger fires.</p>
          </div>
        </div>
          ...
          <div className={cn(
            ...
            <div className="p-4 flex items-center gap-3 md:gap-4 cursor-pointer" onClick={toggleFollowGate}>
              ...
              <div className="flex-1">
                ...
                <p className={cn("text-[11px] md:text-xs font-medium leading-tight", darkMode ? "text-white/40" : "text-gray-500")}>Protect your automation ΓÇö only send the DM after they follow your account</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer pointer-events-none">
                    ...
                    <div className={cn("pt-4 mt-4 border-t", darkMode ? "border-white/10" : "border-gray-100")}>
                      <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input type="text" value={dmAction?.askToFollowBtnText || ''} onChange={(e) => updateDmAction({ askToFollowBtnText: e.target.value })} disabled={readOnly} placeholder="e.g. I've Followed! Γ£à" className={cn("w-full border-2 rounded-xl px-4 py-2 outline-none font-medium text-sm transition-all", darkMode ? "border-white/10 bg-white/5 text-white focus:border-white/20" : "border-gray-200 focus:border-purple-500 text-gray-900")} />
                        </div>
                      </div>
                    </div>
                  </div>
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <p className={cn("text-[10px] font-bold uppercase tracking-wider opacity-40")}>Active ΓÇó 2 Postbacks Max</p>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-8">
                              <div className="text-center space-y-4">
                                <textarea 
                                  ...
                                  placeholder="Hey! Welcome ≡ƒÅ½ How can we help you today?"
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <textarea
                              ...
                              disabled={readOnly}
                              rows={4}
                              placeholder="e.g. Hey! Thanks so much for your comment ≡ƒÆî EverythingΓÇÖs been sent your way Γ£¿"
                            ...
                            />
                            <p className={`text-right text-[10px] font-bold ${darkMode ? 'text-white/20' : 'text-gray-400'}`}>{(dmAction?.title || '').length} / 1000</p>
                          </div>
                                  ...
                                  <div className="p-5 space-y-4">
                                    <div className="space-y-1.5">
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className={cn("text-[9px] font-bold opacity-30")}>{card.title.length} / 1000</span>
                                      </div>
                                      <input 
                                        value={card.title} 
                                        onChange={(e) => {
                                        ...
                                        className={cn("w-full bg-transparent border-b-2 outline-none py-1 text-sm font-bold transition-all", darkMode ? "border-white/10 focus:border-purple-500 text-white" : "border-gray-100 focus:border-purple-500 text-gray-900")}
                                      />
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className={cn("text-[9px] font-bold opacity-30")}>{(card.subtitle || '').length} / 1000</span>
                                      </div>
                                      <input 
                                        value={card.subtitle || ''} 
                                        onChange={(e) => {
                                        ...
                                        className={cn("w-full bg-transparent border-b-2 outline-none py-1 text-sm font-medium transition-all", darkMode ? "border-white/10 focus:border-purple-500 text-white/60" : "border-gray-100 focus:border-purple-500 text-gray-500")}
                                      />
                                    </div>
          ...
          <div className={`p-1.5 md:p-2 mb-4 space-y-1.5 md:space-y-2 transition-colors duration-300 ${darkMode ? '' : 'bg-white border-2 border-purple-100 rounded-[1.5rem]'}`}>
            <div className={`rounded-2xl border-2 transition-all overflow-hidden ${hasLeadManager ? (darkMode ? 'border-orange-500/30 bg-transparent' : 'border-orange-200 bg-orange-50/30') : (darkMode ? 'border-transparent bg-transparent hover:bg-white/[0.04]' : 'border-transparent bg-white hover:bg-gray-50')}`}>
              <div className="p-3 md:p-4 flex items-center gap-3 md:gap-4 cursor-pointer" onClick={toggleLeadManager}>
                ...
                <div className="flex-1">
                  <h3 className={`font-bold text-[14px] md:text-[15px] mb-0.5 md:mb-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}>Save Auto-Leads</h3>
                  <p className={`text-[11px] md:text-xs font-medium ${darkMode ? 'text-white/40' : 'text-gray-400'}`}>Sync collected user information directly to a Google Sheet</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer pointer-events-none">
                ...
                {hasLeadManager && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-5 pb-5 pt-0">
                    <div className={`p-4 rounded-2xl border shadow-sm space-y-4 ${darkMode ? 'bg-transparent border-white/5' : 'bg-white border-orange-100'}`}>
                      <div className="space-y-2">
                        <label className={`text-[10px] font-bold uppercase tracking-wide ${darkMode ? 'text-white/40' : 'text-gray-500'}`}>Google Sheets Connection</label>
                        <label className={`text-[10px] font-bold uppercase tracking-wide px-1 ${darkMode ? 'text-white/40' : 'text-gray-500'}`}>Data to Collect</label>
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (isSelected) newFields.delete(field);
                        ...
                        <div className="space-y-2">
                          ...
                          {isCheckingConnection ? (
                            ...
                            <div className={cn("p-4 rounded-xl border flex items-center justify-between transition-all", darkMode ? "bg-emerald-500/10 border-emerald-500/20" : "bg-emerald-50 border-emerald-200")}>
                               <div className="flex items-center gap-3">
                                 <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                 <div className="flex-1">
                                   <p className={cn("text-[11px] font-black uppercase tracking-wider", darkMode ? "text-emerald-400" : "text-emerald-700")}>Connected to Google</p>
                                   <p className={cn("text-xs font-medium opacity-70", darkMode ? "text-white" : "text-gray-900")}>{connectedEmail}</p>
                                 </div>
                               </div>
                               {!readOnly && (
                                 <button 
                                   onClick={handleDisconnectGoogle}
                                   className={cn("text-[10px] font-bold py-1 px-3 rounded-lg border transition-all", darkMode ? "border-white/10 text-white/40 hover:bg-white/10 hover:text-white" : "bg-white border-gray-200 text-gray-500 hover:text-red-600 hover:border-red-100")}
                                 >
                                   Disconnect
                                 </button>
                               )}
                            ...
                            <button
                              ...
                              disabled={readOnly}
                              className={cn(
                                "w-full flex items-center justify-center gap-2 py-4 rounded-2xl border-2 font-bold text-sm transition-all group",
                                darkMode 
                            ...
                            >
                              <div className={cn("w-7 h-7 rounded-full flex items-center justify-center transition-transform group-hover:scale-110", darkMode ? "bg-white/10" : "bg-white shadow-sm")}>
                                <Globe className={cn("w-4 h-4", darkMode ? "text-blue-400" : "text-blue-600")} />
                              </div>
                              Connect Google Account
                            </button>
                          )}
  
                          <div className={cn("p-4 rounded-2xl border relative transition-all overflow-hidden mt-2", darkMode ? "bg-blue-500/10 border-blue-500/20 text-blue-300" : "bg-blue-50 border-blue-200 text-blue-800")}>
                            <div className="relative z-10 flex items-start gap-3">
                              <Info className="w-5 h-5 shrink-0 mt-0.5" />
                              <div>
                                <h4 className="text-xs font-bold mb-1">OAuth Connection Required</h4>
                                <p className="text-[11px] leading-relaxed opacity-90">
                                  Connecting your Google account allows us to write directly to your private spreadsheets without you having to set them to "Public".
                                </p>
                              </div>
                            </div>
                            {!isGoogleConnected && (
                          ...
                          <label className={`text-[10px] font-bold uppercase tracking-wide block pt-3 ${darkMode ? 'text-white/40' : 'text-gray-500'}`}>Sheet URL</label>
                          <input
                            type="url"
                            value={leadAction?.spreadsheetUrl || ''}
                            onChange={(e) => {
                              if (!readOnly && leadAction) {
                                const newActions = [...actions];
                                const i = newActions.findIndex(a => a.type === 'save_lead');
                                if (i >= 0) {
                                  newActions[i] = { ...newActions[i], spreadsheetUrl: e.target.value } as SaveLeadAction;
                                  onActionsChange(newActions);
                                }
                              }
                            }}
                            ...
                            className={cn(
                              "w-full border-2 rounded-xl px-4 py-3 outline-none font-medium text-sm transition-all",
                              darkMode
                                ? "border-white/10 bg-transparent text-white focus:border-orange-500/30 placeholder:text-white/20"
                                : "border-gray-200 focus:border-orange-400 bg-white text-gray-900 placeholder:text-gray-300"
                            )}
                          />
                        </div>
